...
// 경로에서 인증서를 가져온다
const options = {
    key: fs.readFileSync(path.resolve(process.cwd(), '/etc/letsencrypt/live/lglportfolioweb.tk/privkey.pem'), 'utf-8').toString(),
    cert: fs.readFileSync(path.resolve(process.cwd(), '/etc/letsencrypt/live/lglportfolioweb.tk/cert.pem'), 'utf-8').toString(),
    ca: fs.readFileSync('/etc/letsencrypt/live/lglportfolioweb.tk/fullchain.pem'),
};
...
app.get('/', (req, res) => {
    res.status(200).sendFile(path.join(__dirname + './broadcast.php'))
})
