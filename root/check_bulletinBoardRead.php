...
<div id='writepass'>
    <form action="" method="post">
        <p>비밀번호 <input type="password" name="passwordCheck" /> <input type="submit" value="확인" /><br>* 뒤로 돌아가시려면 아무 번호를 입력하세요.</p>
    </form>
</div>
    ...
    if(isset($_SESSION['userId'])){
        ...
        if(isset($_POST['passwordCheck'])) { //만약 password_check POST값이 있다면
            ...
            if(password_verify($pwk,$bpw)) { //다시 if문으로 DB의 password와 입력하여 받아온 bpw와 값이 같은지 비교를 하고
                ...
                <script type="text/javascript">
                    alert('비밀번호가 틀립니다');
                    location.replace("./bulletinBoard.php?page=<?php echo $page?>");
                </script>
                <?php
            }
        }
