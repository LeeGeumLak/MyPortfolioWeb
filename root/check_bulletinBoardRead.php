...
$sql = mq("select * from bulletinBoard where idxNum='".$bno."'"); /* 받아온 idx값을 선택 */
...
if(isset($_POST['password_check'])) { //만약 password_check POST값이 있다면
    ...
    if(password_verify($pwk,$bpw)) { //다시 if문으로 DB의 password와 입력하여 받아온 bpw와 값이 같은지 비교를 하고
        $pwk == $bpw;
        ?>
        <script type="text/javascript">location.replace("./bulletinBoardRead.php?idxNum=<?php echo $board["idxNum"]; ?>");</script><!-- pwk와 bpw값이 같으면 bulletinBoardRead.php로 보내고 -->
        <?php
    } else{ ?>
